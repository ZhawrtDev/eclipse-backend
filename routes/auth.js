...
router.get("/discord", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (!isMember) {
        return res.redirect("http://eclipse-serverside.vercel.app/error?msg=not_in_guild");
      }
    } catch (error) {
      console.error("Erro ao verificar membros da guilda:", error);
      return res.redirect("http://eclipse-serverside.vercel.app/error?msg=guild_check_failed");
    }
    ...
    try {
      ...
      if (userRoles.length === 0) {
        return res.redirect("http://eclipse-serverside.vercel.app/error?msg=no_role_permission");
      }
    } catch (error) {
      ...
      return res.redirect("http://eclipse-serverside.vercel.app/error?msg=role_fetch_failed");
    }
    ...
    res.redirect(
      `http://eclipse-serverside.vercel.app/auth/?token=${token}&userId=${user.id}&discordId=${id}`
    );
  } catch (error) {
    res.redirect("http://eclipse-serverside.vercel.app/error");
