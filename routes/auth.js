...
router.get("/discord", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (!isMember) {
        return res.redirect("https://eclipse-vite.onrender.com/error?msg=not_in_guild");
      }
    } catch (error) {
      console.error("Erro ao verificar membros da guilda:", error);
      return res.redirect("https://eclipse-vite.onrender.com/error?msg=guild_check_failed");
    }
    ...
    try {
      ...
      if (userRoles.length === 0) {
        return res.redirect("https://eclipse-vite.onrender.com/error?msg=no_role_permission");
      }
    } catch (error) {
      ...
      return res.redirect("https://eclipse-vite.onrender.com/error?msg=role_fetch_failed");
    }
    ...
    res.redirect(
      `https://eclipse-vite.onrender.com/auth/?token=${token}&userId=${user.id}&discordId=${id}`
    );
  } catch (error) {
    res.redirect("https://eclipse-vite.onrender.com/error");
